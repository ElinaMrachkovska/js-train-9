...
function findElementGreaterThan(arr, num) {
  ...
  if (foundElement === undefined) {
    ...
    console.log(`Числа, яке більше ${num}, не знайдено.`);
    return undefined;
  }
...
function removeAndDouble(arr, startIdx) {
  ...
  const removedElements = arr.splice(0, startIdx);
  // За допомогою методу map() подвоюємо кожен елемент масиву.
  const doubledElements = removedElements.map((element) => element * 2);
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((acc, word) => {
